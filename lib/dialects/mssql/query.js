  ...
  getSQLTypeFromJsType(value, TYPES) {
    ...
    if (typeof value === 'number') {
      if (Number.isInteger(value)) {
        ...
        paramType.type = TYPES.Decimal;
        let decimalsPos = value.toString().indexOf('.')
        if (decimalsPos > 0) {
          let decimals = value.toString().length - decimalsPos -1
          if (decimals <=2) {
            paramType.type = TYPES.Money;
          }
        }
        //Default to a reasonable numeric precision/scale pending more sophisticated logic
        paramType.typeOptions = { precision: 19, scale: 15 };
      }
    }
